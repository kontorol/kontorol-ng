...
export class kontorolPlayerComponent implements AfterViewInit, OnDestroy {
	...
	@Input()
	cdnUrl : string = 'http://cdnapi.kontorol.com';
	...
	@Output()
	kontorolPlayerReady = new EventEmitter<any>();
	...
	public Embed():void{
		...
		if (!this.pid || !this.uiconfid || !this.entryid){
			console.warn("kontorol Player::Missing parameters. Please provide pid, uiconfid and entryid.");
		}else {
			// load player lib if doesn't exist
			if (document.getElementById("kontorolPlayerLib") === null) {
				let s = document.createElement('script');
				s.src = `${this.cdnUrl}/p/${this.pid}/sp/${this.pid}00/embedIframeJs/uiconf_id/${this.uiconfid}/partner_id/${this.pid}`;
				s.id = "kontorolPlayerLib";
				s.async = false;
	...
	private doEmbed():void{
		window['kWidget'].embed({
			"targetId": "kontorol_player_" + this.id,
			"wid": "_" + this.pid,
			...
			"readyCallback": (playerID) => {
				...
				this.kontorolPlayerReady.emit(this.kdp);
			}
		});
	}
